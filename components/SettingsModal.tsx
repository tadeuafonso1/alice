...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, settings, onSave, onReset }) => {
  ...
  const [openSections, setOpenSections] = useState({ commands: false, messages: false, customCommands: false });
    ...
    return (
      <div key={key}>
        <div className="flex items-center justify-between mb-1">
          <label htmlFor={`message-text-${key}`} className="block text-sm font-medium text-gray-500 dark:text-gray-400">
        ...
        <textarea
          ...
          className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-md py-2 px-3 focus:ring-2 focus:ring-cyan-500 focus:outline-none text-sm disabled:opacity-50 disabled:cursor-not-allowed"
          rows={2}
  ...
  const renderCommandSetting = (key: CommandKey) => {
    ...
    return (
      <div key={key}>
        <div className="flex items-center justify-between mb-1">
          <label htmlFor={`command-text-${key}`} className="block text-sm font-medium text-gray-500 dark:text-gray-400">
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col text-gray-800 dark:text-gray-200">
